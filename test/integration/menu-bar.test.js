...
describe('Menu bar settings', () => {
    ...
    test('Logo should be clickable', async () => {
        await loadUri(uri);
        await clickXpath('//img[@alt="Bracelet Maker"]');
    ...
    test('(GH#4064) Project name should be editable', async () => {
        await loadUri(uri);
        const el = await findByXpath('//input[@value="Bracelet Maker Project"]');
        await el.sendKeys(' - Personalized');
        await clickText('Costumes'); // just to blur the input
        await clickXpath('//input[@value="Bracelet Maker Project - Personalized"]');
    });
